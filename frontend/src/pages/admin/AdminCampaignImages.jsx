...
function AdminCampaignImages() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      if (editingImage) {
        if (file) payload.url = await uploadImageToCloudinary(file);
        await api.put(`/campaign-images/${editingImage.id}`, payload);
      } else {
        payload.url = await uploadImageToCloudinary(file);
        await api.post("/campaign-images", payload);
      }
      resetForm();
      loadImages();
    } catch (err) {
  ...
  async function toggleActive(image) {
    await api.put(`/campaign-images/${image.id}`, {
  ...
  async function handleDelete(id) {
    await api.delete(`/campaign-images/${id}`);
    loadImages();
  }
  ...
  return (
    <div className="px-6 py-12 max-w-3xl">
      ...
      <h1 className="font-serif text-3xl text-ink mb-8">Campaign Images</h1>
...
export default AdminCampaignImages;
